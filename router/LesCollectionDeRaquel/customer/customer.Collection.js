...
class CustomerCollection {
    ...
    static create(newcollection, result) {
        sql.query("INSERT INTO collection SET ?", newcollection, (err, res) => {
           ...
           winston.info("created collection: ", { id: res.insertId, ...newcollection });
            result(null, { id: res.insertId, ...newcollection });
        });
    }
    static findById(collectionId, result) {
        sql.query(`SELECT * FROM collection WHERE id = ${collectionId}`, (err, res) => {
            if (err) {
                winston.error(err);
                result(err, null);
                return;
    ...
    static updateById(id, customerCollection, result) {
        sql.query(
            ...
            (err, res) => {
               ...
               winston.info("updated customerCollection: ", { id: id, ...customerCollection });
