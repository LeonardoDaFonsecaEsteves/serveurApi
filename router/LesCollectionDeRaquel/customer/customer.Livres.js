...
class CustomerLivre {
    ...
    static updateById(id, customerLivre, result) {
        sql.query(
            "UPDATE livre SET imagesUrl = ?, titre = ?, possede = ?  collection = ?, WHERE livreId = ?",
            [customerLivre.imagesUrl, customerLivre.titre, customerLivre.possede, customerLivre.collection, customerLivre.livreId],
            (err, res) => {
                if (err) {
                    winston.error(err);
                    result(null, err);
                ...
                winston.info("updated customerLivre: ", { id: id, ...customerLivre });
