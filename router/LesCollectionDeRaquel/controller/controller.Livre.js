...
exports.create = (req, res) => {
    ...
    name = name.replace(/[^A-Z0-9]+/ig, "-") + '.jpg'
    ...
    CustomerLivre.create(customer, (err, data) => {
        if (err)
            res.status(500).send({
                message:
                    err.message || "Some error occurred while creating the livres."
            });
        else res.send(data);
    });
};
    ...
    CustomerLivre.findById(req.params.livreId, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                ...
                res.status(500).send({
                    message: "Error retrieving livres with id " + req.params.livreId
                });
            }
        } else res.send(data);
    });
        ...
        (err, data) => {
            if (err) {
                if (err.kind === "not_found") {
                    ...
                    res.status(500).send({
                        message: "Error updating livres with id " + req.params.livreId
                    });
                }
            } else res.send(data);
        }
    ...
    CustomerLivre.remove(req.params.livreId, (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            ...
            res.status(500).send({
              message: "Could not delete livres with id " + req.params.livreId
            });
          }
