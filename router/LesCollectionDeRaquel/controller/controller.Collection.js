  ...
  CustomerCollection.findById(req.params.collectionId, (err, data) => {
    if (err) {
      if (err.kind === 'not_found') {
        ...
        res.status(500).send({
          message:
                        'Error retrieving collections with id ' +
  ...
  CustomerCollection.updateById(
      ...
      (err, data) => {
        if (err) {
          if (err.kind === 'not_found') {
            res.status(404).send({
              message: `Not found collections \
               with id ${req.params.collectionId}.`,
            });
          } else {
            res.status(500).send({
              message:
                            'Error updating collections with id  ' +
                            req.params.collectionId,
            });
          }
        } else res.send(data);
  ...
  CustomerCollection.remove(req.params.collectionId, (err, data) => {
    if (err) {
      if (err.kind === 'not_found') {
        ...
        res.status(500).send({
          message:
                        'Could not delete Collections with id ' +
                        req.params.collectionId,
        });
      }
  ...
  CustomerCollection.removeAll((err, data) => {
    if (err) {
      res.status(500).send({
        message:
                    err.message ||
                    'Some error occurred while removing all collections.',
      });
    } else {
      res.send({message: 'All collections were deleted successfully!'});
    }
  });
};
